...
import static com.obss.go.util.GamePlayUtils.isCellAvailableFor;
import static com.obss.go.util.GamePlayUtils.isCellEmpty;
import static com.obss.go.util.GamePlayUtils.isGroupBreathing;
...
public class GamePlayService implements IGamePlayService, EventListener {
	...
	public void playTurn(CELL_X x, CELL_Y y) throws CellNotEmptyException, CellNotBreathingException, KOException {
		...
		if (isCellAvailableFor(Game.getCells().get(activeCellKey), Game.getActivePlayer()) == false) {
	...
	private void evaluateGroups(CELL_STATUS groupColor) {
		...
		while (it.hasNext()) {
			...
			if (stoneGroupEntry.getValue().getColor().equals(groupColor) && isGroupBreathing(stoneGroupEntry.getValue()) == false) {			
